...
def index_to_date(index: int) -> date:
    ...
    1 -> BASE_DATE, 2 -> BASE_DATE + 1 день, и т.д.
    """
    if index < 1:
...
def date_to_start_time_rfc3339(d: date) -> str:
    """
    Формирует строку времени старта в RFC3339.
    При необходимости поменяй время '21:30:00-04:00'.
    """
...
def parse_range(arg: str) -> tuple[int, int]:
    """
    Разбирает аргумент вида '275-282' в (275, 282).
    """
    if "-" not in arg:
...
def choose_playlists_for_date(d: date) -> list[str]:
    """
    Возвращает список плейлистов для данной даты.

    Правило:
    - всегда общий плейлист (если задан);
    - если дата — воскресенье (weekday == 6):
        → Full version;
      иначе:
        → 1/2 version.
    """
    ...
    if GENERAL_PLAYLIST_ID:
        playlists.append(GENERAL_PLAYLIST_ID)

    is_sunday = d.weekday() == 6  # Monday = 0, Sunday = 6

    if is_sunday:
        if FULL_PLAYLIST_ID:
            playlists.append(FULL_PLAYLIST_ID)
    else:
        if HALF_PLAYLIST_ID:
            playlists.append(HALF_PLAYLIST_ID)
...
def main():
    ...
    for index in range(start, end + 1):
        try:
            ...
            print(f"[{index}] ошибка вычисления даты: {e}")
            continue
        ...
        if not thumb_path.exists():
            print(
                f"[{index}] ВНИМАНИЕ: файл обложки не найден: {thumb_path}. "
                "Пропускаю этот день."
            )
            continue
        ...
        print(f"    Дата/время (RFC3339): {start_time}")
        print(f"    Обложка: {thumb_path}")
        print(f"    Плейлисты: {playlists or '— (нет)'}")
